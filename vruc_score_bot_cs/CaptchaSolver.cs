            ...
            using (var image = ResizeBitmap(image_, image_.Width + 2, image_.Height + 2))
            {
                ...
                var result = Enumerable.Range(0, image.Width)
                    ...
                    ).Aggregate((false, new List<(int cstart, int cend)>()), (x, y) =>
                    {
                        ...
                        {
                            if (x.Item1 == false)
                                x.Item2.Add((y.Item1, -1));
                            else
                                x.Item2[x.Item2.Count - 1] = (x.Item2[x.Item2.Count - 1].cstart, y.Item1);
                        }
                            ...
                            ).Aggregate((false, new List<(int rstart, int rend)>()), (x, y) =>
                            {
                                if (x.Item1 != y.Item2)
                                {
                                    if (x.Item1 == false)
                                        ...
                                        x.Item2[x.Item2.Count - 1] = (x.Item2[x.Item2.Count - 1].rstart, y.Item1);
                                }
                    ...
                    .Select(t =>
                    {
                        ...
                        using (var chr = image.Clone(new Rectangle(cstart, rstart, cend - cstart, rend - rstart), image.PixelFormat))
                        {
                            ...
                            {
                                return normalized.Zip(pair.Value, (x, y) => (x - y) * (x - y)).Aggregate(0.0, (x, y) => x + y);
                            }).FirstOrDefault().Key;
                        }
                    })
                    .Aggregate("", (x, y) => x + y);
